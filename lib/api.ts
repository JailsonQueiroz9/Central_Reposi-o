export const api = {
  ...
  post: async (action: string, data: any = {}) => {
    ...
    if (isInvalidUrl) {
      if (!url) {
        console.warn('[API] NEXT_PUBLIC_APPS_SCRIPT_URL não está configurado no .env. Usando modo offline/mock.');
      } else {
    ...
    try {
      ...
      // Fallback crítico: Se a rede falhar, usamos dados mockados
      // para que o usuário consiga ao menos visualizar a interface sem erros no console.
      ...
      if (isNetworkError) {
        console.warn(`[API] Falha de conexão com a API para [${action}]. Usando dados mockados como fallback.`);
        return mockResponse(action, data);
      }
      
      console.error(`[API] Erro na chamada da API [${action}]:`, error);
      throw error;
    }
  }
};
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      switch (action) {
        ...
        return { success: true, id: data.id || 'mock-id-123' };
