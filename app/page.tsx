...
import { LayoutDashboard, FileText, Activity, MessageCircle, Settings, Menu, X, LogOut, Loader2, Download } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { api } from '@/lib/api';
import { dataCache } from '@/lib/cache';

import Cadastro from '@/components/views/Cadastro';
import CadastroAlmx from '@/components/views/CadastroAlmx';
...
type ViewType = 'painel' | 'cadastro' | 'cadastroAlmx' | 'followup' | 'chat' | 'configuracao' | 'cadastroEntrega';

const menuItems = [
  { id: 'painel', label: 'Painel (Status)', icon: LayoutDashboard, perm: 'painel' },
  { id: 'cadastro', label: 'Cadastro', icon: FileText, perm: 'cadastro' },
  { id: 'cadastroAlmx', label: 'Cadastro Almx', icon: FileText, perm: 'almx' },
...
export default function Home() {
  ...
  const handleLoginSuccess = () => {
    ...
    if (storedUser) {
      ...
      if (pObj) {
        if (pObj.painel) setActiveView('painel');
        else if (pObj.cadastro) setActiveView('cadastro');
        else if (pObj.almx) setActiveView('cadastroAlmx');
        else if (pObj.followup) setActiveView('followup');
        else if (pObj.chat) setActiveView('chat');
        else if (pObj.config) setActiveView('configuracao');
      }
    }
  ...
  const filteredMenuItems = useMemo(() => {
    ...
    return menuItems.filter(item => permissions[item.perm] === true);
  }, [permissions]);

  // Só considera o sistema "carregado" se:
  // 1. O carregamento inicial terminou
  // 2. Se autenticado, as permissões já foram processadas e NÃO estão vazias (ou se realmente não tem nenhuma)
  const isAppReady = !isInitialLoading && (!isAuthenticated || (isAuthenticated && permissions !== null && filteredMenuItems.length > 0));
    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-gray-50 gap-4">
        <div className="relative">
          <Loader2 className="animate-spin text-red-800" size={56} />
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-2 h-2 bg-red-800 rounded-full animate-ping"></div>
          </div>
        </div>
        <div className="flex flex-col items-center gap-1">
          <p className="text-gray-800 font-bold text-lg tracking-widest">CONTROLE DE REP</p>
          <p className="text-gray-500 font-medium animate-pulse text-sm">Sincronizando ambiente e permissões...</p>
        </div>
      </div>
    );
  }
  ...
  const renderView = () => {
    switch (activeView) {
      ...
      case 'cadastroAlmx': return <CadastroAlmx />;
  ...
  return (
    <div className="flex h-screen w-full bg-gray-100 overflow-hidden">
      ...
      <motion.aside
        ...
        className="fixed md:relative z-50 h-full bg-[#8B0000] text-white flex flex-col shadow-2xl overflow-hidden whitespace-nowrap print:hidden"
      >
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-6 py-3 transition-colors ${
                  isActive 
                    ? 'bg-white/20 border-r-4 border-orange-400 font-semibold' 
        ...
        <div className="p-4 border-t border-white/10 space-y-4">
          {/* User Profile Info */}
          <div className="flex items-center gap-3 px-2 py-2 bg-white/5 rounded-lg border border-white/10">
          ...
          <button 
            ...
            className="w-full flex items-center gap-3 px-2 py-2 hover:bg-white/10 text-white/80 hover:text-white transition-colors rounded print:hidden"
          >
            <LogOut size={20} />
            <span>Sair</span>
          </button>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full">
        {/* Top Header */}
        <header className="h-14 bg-white border-b border-gray-200 flex items-center px-4 shadow-sm z-10 print:hidden">
          <button 
          ...
          <div className="ml-4 font-semibold text-gray-800 capitalize">
